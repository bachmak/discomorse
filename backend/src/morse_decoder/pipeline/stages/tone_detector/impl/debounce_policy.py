...
class KeyingDelayPolicy:
    ...
    The two sides wait differently, because their mistakes cost differently: a
    gap torn into a mark splits one element into two, while a mark stretched by
    a few milliseconds only shifts an edge the timing stage reads as a ratio.
    So a key that falls is believed almost at once, and a key that lifts is made
    to hold long enough that a dropout inside a mark is bridged rather than sent on.
    """
