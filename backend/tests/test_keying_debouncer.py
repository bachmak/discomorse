...
async def test_the_key_falls_only_once_it_has_been_held_down_past_the_rise_delay(
    seconds: float, want: int
) -> None:
    assert edges(await debounced(blip(True, seconds))) == want
...
async def test_a_delay_is_a_stretch_of_time_and_not_a_count_of_readings(
    ...
    assert edges(await debounced(line)) == want


async def test_a_key_flapping_faster_than_the_delays_never_reaches_the_output() -> None:
    line = (
        KeyTimeline()
        .hold(False, SETTLE_SECONDS)
    ...
    assert not any(await debounced(line))


async def test_a_mark_torn_by_a_dropout_still_reads_as_one_mark() -> None:
    line = (
        KeyTimeline()
        .hold(False, SETTLE_SECONDS)
    ...
    assert edges(await debounced(line)) == 2


async def test_the_gap_between_two_elements_lives_through_the_debouncer() -> None:
    line = (
        KeyTimeline()
        .hold(False, SETTLE_SECONDS)
        .hold(True, _DIT_SECONDS)
...
async def test_a_stream_read_in_two_parts_reads_as_one_stream() -> None:
    ...
    head = await debounced(readings[:split], keying_debouncer=reader)
    tail = await debounced(readings[split:], keying_debouncer=reader)

    assert head + tail == await debounced(readings)
